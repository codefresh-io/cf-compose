...
DockerComposeTransformer.prototype.toOutput = function () {
    ...
    yamlString = this.injectEnvVars(yamlString);
...
DockerComposeTransformer.prototype.injectEnvVars = function(content) {
    if (!this.opts.envVars || this.opts.envVars.length === 0) {
        return content;
    }
    this.opts.envVars.forEach(function (envVar) {
        var keyPattern = new RegExp(_.escapeRegExp("$" + envVar.key), "g");
        content = content.replace(keyPattern, envVar.value);
    });
